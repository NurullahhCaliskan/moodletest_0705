...
/**
 * Privacy Subsystem implementation for theme_wellko.
 *
...
namespace theme_wellko\privacy;
...
class provider implements
    ...
    public static function export_user_preferences(int $userid) {
        ...
        if (isset($draweropennavpref)) {
            $preferencestring = get_string('privacy:drawernavclosed', 'theme_wellko');
            if ($draweropennavpref == 'true') {
                $preferencestring = get_string('privacy:drawernavopen', 'theme_wellko');
            }
        ...
        if (isset($draweropenindexpref)) {
            $preferencestring = get_string('privacy:drawerindexclosed', 'theme_wellko');
            if ($draweropenindexpref == 1) {
                $preferencestring = get_string('privacy:drawerindexopen', 'theme_wellko');
            }
            \core_privacy\local\request\writer::export_user_preference(
                'theme_wellko',
        ...
        if (isset($draweropenblockpref)) {
            $preferencestring = get_string('privacy:drawerblockclosed', 'theme_wellko');
            if ($draweropenblockpref == 1) {
                $preferencestring = get_string('privacy:drawerblockopen', 'theme_wellko');
            }
            \core_privacy\local\request\writer::export_user_preference(
                'theme_wellko',

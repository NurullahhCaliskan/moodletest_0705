...
function scheduler_print_schedulebox(scheduler $scheduler, $studentid, $groupid = 0) {
    ...
    foreach ($availableslots as $slot) {
        ...
        $startdatestr = $startdatecnv;
        $starttimestr = $starttimecnv;
    ...
    foreach ($slots as $slot) {
        ...
        foreach ($slot->get_appointments() as $app) {
            ...
            foreach($appointments as $appointment)
            {
                    $x=1;
                    $currentTime=time(); 
                    //empty($slot->appointmentlocation)&& $appointment->attended==0 && 
                    //$appointment->is_calendar_event_created=='0'
                    if($appointment->timecreated>=($currentTime-(5)) && $appointment->is_calendar_event_created==0)
                    //
                    {
                    $appointment->is_calendar_event_created=1;
                    // Slotun başlangıç zamanını al

                    $slot_duration = $slot->duration;
        
                    // Bitiş zamanını hesapla
                    $endtime = $slot->starttime + $slot_duration;

...
abstract class scheduler_slotform_base extends moodleform {
    ...
    protected function add_minutes_field($name, $label, $defaultval, $minuteslabel = 'minutes') {
        ...
        // Create select element with predefined duration options
        $select = $mform->createElement('select', $name, get_string($label, 'scheduler'));
        $select->addOption('30', 30);
        $select->addOption('60', 60);
        $select->addOption('90', 90);
        $select->addOption('120', 120);
        $group[] =& $select;
    ...
    protected function add_minutes_field2($name, $label, $defaultval, $minuteslabel = 'minutes') {
        $mform = $this->_form;
        $group = array();
...
class scheduler_addsession_form extends scheduler_slotform_base {
    ...
    protected function definition() {
        ...
        // Break between slots.
        $this->add_minutes_field2('break', 'break', 0, 'minutes');

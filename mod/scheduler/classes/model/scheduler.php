...
class scheduler extends mvc_record_model {
    ...
    protected function fetch_slots($wherecond, $havingcond, array $params, $limitfrom='', $limitnum='', $orderby='') {
        ...
        $teacher = $this->get_first_course_teacher_id($courseid);
    ...
    protected function get_course_teacher_ids($course) {
        ...
        $params = array($course);
        $teacher_ids = $DB->get_records_sql($sql, $params);
        
        return array_keys($teacher_ids);
    }
    
    protected function get_first_course_teacher_id($courseid) {
        global $DB;
        $sql = "SELECT ra.userid
                FROM {role_assignments} ra
                JOIN {context} ctx ON ra.contextid = ctx.id
        ...
        // get_field_sql metodunu kullanarak sorgu sonucundaki ilk değeri al
        $first_teacher_id = $DB->get_field_sql($sql, $params);
        
        return $first_teacher_id;
    }
    /** EYK düzeltmeleri */
    ...
    //EYK    $slotdata = $DB->get_record('scheduler_slots', array('id' => $id, 'schedulerid' => $this->id), '*', MUST_EXIST);
    ...
    public function get_all_slots($limitfrom='', $limitnum='') {
        return $this->fetch_slots('', '', array(), $limitfrom, $limitnum, 's.starttime DESC');
    }
    ...
    public function get_attended_slots_for_student($studentid) {
        ...
        $slots = $this->fetch_slots($wherecond, '', $params, '', '', 's.starttime DESC');
    ...
    public function get_slots_available_to_student($studentid, $includefullybooked = false) {
        ...
        $order = 's.starttime DESC, s.duration ASC, s.teacherid';
    ...
    public function get_slots_for_teacher($teacherid, $groupid = 0, $limitfrom = '', $limitnum = '', $timerange = 0) {
        ...
        return $this->fetch_slots($where, '', $params, $limitfrom, $limitnum, 's.starttime DESC, s.duration DESC, s.teacherid');
    }
    ...
    public function delete_appointment($appointmentid) {
        ...
        //// EYK
        //$eventtype = "SSstu:{$slot->data->id}".
        //$DB->delete_records('event', array('eventtype' => "SSstu:92"));
        //$eventtype = "SSsup:{$slot->data->id}".
        //$DB->delete_records('event', array('eventtype' => "SSsup:92"));
        //$DB->delete_records('event', array('eventtype' => $slot->get_student_eventtype()));
        /** 
         * Burada elde olan appointmentlar ve database kıyaslanıp eklenenler güncelleniyor
         * db de yoksa ekleniyor
         * db ki silinmiyorsa güncelleniyor
         * ya da siliniyor
         * bir appointment silindiğinde ait olduğu slota ait eventleri takvimden silmek 
         * için bir fonksiyona ihtiyaç var 
         */ 
